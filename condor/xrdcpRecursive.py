...
for sourceFile in getFileList(options.source):
  ...
  print "xrdcp %s root://cmseos.fnal.gov/%s" % (sourceFile, targetFile)
  #print getoutput("xrdcp --verbose --path %s root://cmseos.fnal.gov/%s" % (sourceFile, targetFile))
  print getoutput("xrdcp --force %s root://cmseos.fnal.gov/%s" % (sourceFile, targetFile))
