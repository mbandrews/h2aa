...
#
#    if ma_bins is not None:
#        h[sr_k+'rebin'] = rebin2d(h[sr_k], ma_bins)
#        h[sb2sr_k+'rebin'] = rebin2d(h[sb2sr_k], ma_bins)
#
#    k = sr_k+'rebin'
#    for ix in range(1, h[k].GetNbinsX()+1):
#        for iy in range(1, h[k].GetNbinsY()+1):
...
#            binc = h[k].GetBinContent(ix, iy)
#            h[k].SetBinContent(ix, iy, binc)
#    plot_2dma(sr_k+'rebin', "", xtitle, ytitle, ztitle, zrange, do_log=True)
#    plot_2dma(sb2sr_k+'rebin', "", xtitle, ytitle, ztitle, zrange, do_log=True)
...
varvec = np.array([
    ...
    #np.sqrt(9.077080e-04) * np.array([ -0.00374573286076,-0.588997012373,-0.291085420381,0.439677214411,0.557816813279,0.25270955046 ]),
    #np.sqrt(6.778611e-04) * np.array([ 0.0977009154996,0.238113223834,-0.674918990025,-0.159048080323,-0.256613549096,0.622168928199 ]),
    #np.sqrt(3.981678e-04) * np.array([ -0.122684383918,0.0251122495223,0.035011623697,-0.775490096368,0.609546251947,0.100799703602 ]),
    #np.sqrt(5.121247e-06) * np.array([ -0.718049540711,-0.0822616270925,-0.50347331491,-0.0535073729495,-0.104439158588,-0.458674095802 ]),
    #np.sqrt(4.902121e-05) * np.array([ 0.518825328491,-0.579962784712,-0.240860814393,-0.378752971241,-0.284332078266,-0.334889490728 ]),
    #np.sqrt(3.797069e-05) * np.array([ -0.436593836579,-0.502623298337,0.383442968922,-0.183537226537,-0.399613547573,0.465134636944 ]),
    ## a0 inv, a1 inv
