...
def edit_shift(shift):
    #if 'Dn' or 'dn' in shift:
    #    return shift.replace('Dn','Down').replace('dn', 'Down')
    #if 'up' in shift:
    #    return shift.replace('up', 'Up')
    return k.replace('dn','Down').replace('Dn', 'Down').replace('up','Up')
...
for i,syst in enumerate(systs):
    ...
    ksysts = [k for k in ksysts if '_rebin_ma1' in k]
    #print(ksysts)
    ksysts = [k for k in ksysts if limit_blind in k]
    #print(ksysts)
    for shift in ['Down', 'Up']:
        #ksyst_shift = [k for k in ksysts if shift in k][0]
        ksyst_shift = [k for k in ksysts if shift in edit_shift(k)][0]
...
for shift in ['Nom', 'Obs']:
    ksyst_shift = 'data_%s_%s_%s_rebin_ma1'%('sb2sr' if shift == 'Nom' else 'sr', limit_blind, key_2d)
    #print(ksyst_shift, h[ksyst_shift].Integral())
    kout = 'data_obs' if shift == 'Obs' else 'bkg'
    hout[kout] = h[ksyst_shift].Clone()
    hout[kout].SetName(kout)
...
for ma in ma_pts:
    ...
    for run in runs:
        ...
        for shift in ['Obs']:
            #print('shift:',shift)
            #ksyst_shift = [k for k in ksysts if shift in k][0]
            ksyst_shift = '%s_%s_%s_%s_rebin_ma1'%(sample, 'sr', limit_blind, key_2d)
            #h4g-mA0p1GeV_sr_offdiag_lo_hi_ma0vma1_rebin_ma1
            #print(ksyst_shift, h[ksyst_shift].Integral())
            #kout = 'h4g'
            #kout = 'h4g_%s'%ma
            kout = 'h4g_%s_%s'%(ma, run)
