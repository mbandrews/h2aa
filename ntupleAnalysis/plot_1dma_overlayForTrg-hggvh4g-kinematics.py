...
def draw_hist_1dma_overlay_ratio(k_, h, hc, c, l, hatch, legend, it, ymax_=None):
    ...
    #h[k] = set_hist(h[k], "m_{a,pred} [GeV]", "%s_{a} / 25 MeV"%('f' if normalize else 'N'), "")
    h[k] = set_hist(h[k], "m_{a,pred} [GeV]", "%s_{a}"%('f' if normalize else 'N'), "")
    ...
    if it == 0:
        ...
        if k.split('_')[-1] == 'etaxy':
            h[k].GetXaxis().SetRangeUser(-1.6, 1.6)
        elif k.split('_')[-1] == 'r9xy':
            h[k].GetXaxis().SetRangeUser(0.2, 1.2)
    ...
    if it == nit - 1:
        ...
        #else:
        legend = ROOT.TLegend(0.35-0.2, 0.75, 0.92-0.2, 0.88) #(x1, y1, x2, y2)
        ...
        legend.AddEntry(khgg, 'A: H#rightarrow4#gamma #cap !H#rightarrow#gamma#gamma #pm stat%s'%(' #pm syst' if applySF else ''), "le")
        legend.AddEntry(kh4g, 'B: H#rightarrow4#gamma #cap  H#rightarrow#gamma#gamma #pm stat%s'%(' #pm syst' if applySF else ''), "le")
    ...
    if it == 0:
        ...
        fUnity.GetXaxis().SetTitle(xtitle[k.split('_')[-1]])
        ...
        fUnity.GetYaxis().SetTitle("A/B")
        #fUnity.GetYaxis().SetRangeUser(1.-dY,1.+dY)
        #fUnity.GetXaxis().SetRangeUser(0., 1.2)
        #print('BinLowEdge:', h[k].GetBinLowEdge(1), h[k].GetBinLowEdge(h[k].GetNbinsX()+1))
        #fUnity.GetXaxis().SetRangeUser(h[k].GetBinLowEdge(1), h[k].GetBinLowEdge(h[k].GetNbinsX()+1))
    ...
    if it == nit-1:
        outfile = 'Plots/1dma_trgs/QvRun_%so%s.pdf'%(khgg, kh4g)
...
normalize = False
normalize = True
if normalize and ynorms[keys[0]] >= 0.1:
    #ymaxs = {'0p1':0.07, '0p4':0.09, '1p0':0.08,  '1p2':0.08}
    #ymax_norm = 0.4
    #ymax_norm = ynorms[keys
...
for k in keys:
    for m in mas:
        ...
        k0 = h.keys()[0]
        if k == 'etaxy':
            fUnity = ROOT.TF1("fUnity","[0]", -1.6, 1.6)
        elif k == 'r9xy':
            fUnity = ROOT.TF1("fUnity","[0]", 0.2, 1.2)
        else:
            fUnity = ROOT.TF1("fUnity","[0]", h[k0].GetBinLowEdge(1), h[k0].GetBinLowEdge(h[k0].GetNbinsX()+1))
        #pUp.Draw()
        #pDn.Draw()
        #pUp.SetMargin(13.e-02,3.e-02,2.e-02,9.e-02) # (Float_t left, Float_t right, Float_t bottom, Float_t top)
        #pDn.SetMargin(13.e-02,3.e-02,36.e-02,2.e-02) # (Float_t left, Float_t right, Float_t bottom, Float_t top)
