...
nFiles = int(np.ceil(nEvts/5.e6))
print('%s: splitting into %d files'%(args.sample, nFiles))
if nFiles > 1:
    fs = {iF: open("%s/%s%d_zee_ggskim_event_list.txt"%(args.outdir, args.sample, iF),"w+") for iF in range(nFiles)}
else:
    fs = {0: open("%s/%s_zee_ggskim_event_list.txt"%(args.outdir, args.sample),"w+")}
