...
def create_hists(h):
    ...
    k = 'eta0'
    h[k] = ROOT.TH1F(k, k, 50, -1.442, 1.442)
    k = 'eta1'
    h[k] = ROOT.TH1F(k, k, 50, -1.442, 1.442)
...
def fill_hists(h, tree, wgt, outvars=None, pt_wgts=None, pt_edges=None, fma=None, puRange=None):

    h['wgt'].Fill(wgt)

    pu_ = -1.
    if tree.isData:
        if puRange is not None:
            pu_ = get_pu(tree.run, tree.lumis, puRange)
    # if mc, do not use PU json (puRange) info (for data only)
    else:
        pu_ = get_pumc(tree)
    ...
    if outvars is not None:
        ...
        if 'phoTagIdxs' in outvars.keys():
            for i,idx in enumerate(outvars['phoTagIdxs']):
                ...
                h['eta0'].Fill(tree.phoEta[idx], wgt)
        ...
        if 'phoProbeIdxs' in outvars.keys():
            for i,idx in enumerate(outvars['phoProbeIdxs']):
                ...
                #fma.write('%f:%f\n'%(tree.ma[idx], tot_wgt))
                #fma.write('%f:%f:%f\n'%(tree.ma[idx], tree.phoCalibEt[idx], pu_))
                #fma.write('%f:%f:%f:%f\n'%(tree.ma[idx], tree.phoCalibEt[idx], tree.phoEta[idx], pu_))
                fma.write('%f:%f:%f:%f:%d:%f\n'%(tree.ma[idx], tree.phoCalibEt[idx], tree.phoSCEta[idx], tree.phoR9Full5x5[idx], idx, pu_))
                #print('%f:%f:%f\n'%(tree.ma[idx], tree.phoCalibEt[idx], pu_))
                h['maxy'].Fill(tree.ma[idx], tot_wgt)
                h['eta1'].Fill(tree.phoEta[idx], wgt)
...
def get_pu(runId, lumiId, inputPileupRange, minbXsec=69200): # minbXsec [microbarn], run2 default: https://twiki.cern.ch/twiki/bin/view/CMS/PileupJSONFileforData
    pu = -1.
